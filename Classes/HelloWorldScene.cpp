...
void HelloWorld::adjustPosition(int type) {
	...
	case COLLISION_WALL: ResultDNA(1); break;
	case COLLISION_ENDPOINT: ResultDNA(3); break;
	}
}

int HelloWorld::CheckCollision()
{
	...
	CCTMXObjectGroup *objectGroup = map->objectGroupNamed("obj1");
	if (objectGroup != NULL) {
		ValueMap endPoint = objectGroup->getObject("EndPoint");

		float ex = endPoint["x"].asFloat();
		float ey = endPoint["y"].asFloat();
		float ew = endPoint["width"].asFloat();
		float eh = endPoint["height"].asFloat();
		Rect rect = Rect(ex, ey, ew, eh);
		if (car->getBoundingBox().intersectsRect(rect)) return COLLISION_ENDPOINT;
	}
	...
	for (int i = 0; i < 9; i++) {
		if (cx + tmp1[i] < layer->getLayerSize().width && cx + tmp1[i] >= 0 && cy + tmp2[i] >= 0 && cy + tmp2[i] <= layer->getLayerSize().height) {
			...
			if (tileGid == 2) if (car->getBoundingBox().intersectsRect(tile->getBoundingBox())) return COLLISION_WALL;
		}
	}
...
{
	string filename = int2string(nowG) + ".txt";
	...
	string log;
	for (int i = 0; i < DNA_CNT; i++) {
		log = "@ list[" + int2string(i) + "] @";
		log.append("\nfit : " + double2string(list[i]->fit));
		log.append("\ntime : " + double2string(list[i]->time));
		log.append("\nnow : " + int2string(list[i]->now));
		log.append("\nMutation : " + int2string(list[i]->isMutate));
		log.append("\ngenesize : " + int2string(list[i]->gene.size()));
		log.append("\ngene : ");
		log.append("[" + double2string(list[i]->gene[0].angle) + ", " + double2string(list[i]->gene[0].interval) + "]");
		for (int j = 1; j < list[i]->gene.size(); j++) log.append(", [" + double2string(list[i]->gene[j].angle) + ", " + double2string(list[i]->gene[j].interval) + "]");
		log.append("\n==========================================\n\n");
...
void HelloWorld::ResultDNA(int op)
{
	...
	else if (op == 3) str = "[Driving Success]\n";
